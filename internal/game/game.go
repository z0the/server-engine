...
type Game struct {
	...
	characters []*Player
	events     chan hubber.IRequest
	msgPipe    chan<- hubber.IResponse
}

func NewGame(logger *logrus.Logger, characters []*Player, msgPipe chan<- hubber.IResponse) *Game {
	return &Game{
		logger:     logger,
		characters: characters,
		events:     make(chan hubber.IRequest, 10),
		msgPipe:    msgPipe,
	}
}

func (g *Game) HandleRequest(event hubber.IRequest) {
	g.events <- event
}
...
func (g *Game) handleMove(event hubber.IRequest) {
